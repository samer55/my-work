...
console.log(this.state.dataArray);
 ...
 this.filterfun()
  ...
  componentDidMount() {
    ...
    const range = this.getGeohashRange(this.props.appStore.Myregion.latitude, this.props.appStore.Myregion.longitude, this.state.value)
if (this.state.arrrofcat.length>0) {
  var ref2 = firebaseApp.firestore().collection(this.state.namedata) .where("hash", ">=", range.lower)
    .where("hash", "<=", range.upper).limit(this.state.limit1).onSnapshot(this.resent)

}else if(this.state.namedata!=='Offers'){
  var ref2 = firebaseApp.firestore().collection(this.state.namedata)    .where("hash", ">=", range.lower)
    .where("hash", "<=", range.upper).limit(this.state.limit1)
.onSnapshot(this.resent)

}
else if (this.state.namedata==='Offers') {
  var ref2 = firebaseApp.firestore().collection("Offers").limit(this.state.limit1)
.onSnapshot(this.resent)
}
let item=[]
let num = 0
...
var ref = firebaseApp.firestore().collection('Services').onSnapshot(this.onbusiness)
    ...
    onbusiness = (querySnapshot) => {
      ...
      const newData = boards.filter(item => {
        const itemData = item.name;
         ...
         if (itemData&&itemData.indexOf(textData) === -1) {
         return  true
         }else {
         return  false
         }
        ...
        if (newData&&doc.data().special) {
          boards.push({
      id:doc.data().writerId,
            name:doc.data().Categories,
          ar:doc.data().Categories});
        }


      });

this.setState({specialarr:boards})    }
  loadmore = ()=>{
    ...
    const range = this.getGeohashRange(this.props.appStore.Myregion.latitude, this.props.appStore.Myregion.longitude, this.state.value)
if (this.state.arrrofcat.length>0) {
  var ref2 = firebaseApp.firestore().collection(this.state.namedata).orderBy('hash').startAfter(this.state.lastid.data().hash) .where("hash", ">=", range.lower)
    .where("hash", "<=", range.upper).limit(this.state.limit1).onSnapshot(this.moreresent)

}else if(this.state.namedata!=='Offers'){
  var ref2 = firebaseApp.firestore().collection(this.state.namedata)  .orderBy('hash').startAfter(this.state.lastid.data().hash)  .where("hash", ">=", range.lower)
    .where("hash", "<=", range.upper).limit(this.state.limit1)
.onSnapshot(this.moreresent)

}
else if (this.state.namedata==='Offers') {
  var ref2 = firebaseApp.firestore().collection("Offers").orderBy('postuid').startAfter(this.state.lastid.data().postuid).limit(this.state.limit1)
.onSnapshot(this.moreresent)
}
...
this.setState({swipeablePanelActive:false})
  ...
  resent = (querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
      ...
      if (this.state.sec=='All'||this.state.sec==doc.data().type||(this.state.sec=='Special'&&doc.data().special==true)) {
        if (this.state.selectedText.length>0) {
...
<Body>
<Text style={[gStyle.text[theme], gStyle.Titleleft,{paddingHorizontal: 10,fontSize: 15}]}>{this.state.namedata=='Business'?this.props.appStore.arabic?'مشاريع واعمال':'Business':this.state.namedata=='Services'?this.props.appStore.arabic?'خدمات':'Services':this.state.namedata=='Jobs'?this.props.appStore.arabic?'وظائف':'Jobs':this.state.namedata=='Offers'?this.props.appStore.arabic?'عروض':'Offers':this.state.namedata}</Text>
</Body>
<Right>
{this.state.namedata==='Offers'?null:<Item>
  <TouchableOpacity onPress={()=>navigation.navigate('Nearby',{types:this.state.namedata})} style={{marginHorizontal: 10}} >
  <Icon name="map" type="FontAwesome" style={{color: '#388e3c'}}/>
...
if (item.adsactive) {
  if (new Date().getTime() > item.adsactive) {
    console.log("trueee");
    firebaseApp.firestore().collection(this.state.namedata).doc(item.postuid).update({active:false})
  }
}
...
{this.props.appStore.arabic?'ترتيب حسب':'Sort by'}
       ...
       <Picker.Item label={this.props.appStore.arabic?'الاقل سعرا':"Lower price"} value="price" />
       <Picker.Item label={this.props.appStore.arabic?'الاعلى سعرا':"Higher Price"} value="hprice" />
       <Picker.Item label={this.props.appStore.arabic?'الاقدم':"Oldest"} value="Oldest" />
...
{this.props.appStore.arabic?'النوع':'Type'}
        ...
        <Text style={{color:this.state.sec===''?'#fff':'black'}}>{this.props.appStore.arabic?'الجميع':'All'}</Text>
        </Button>
        {
        this.state.listedsection.map((category, index) => {
            ...
            return (
              ...
              <Text style={{color:category==this.state.sec?'#fff':'black'}}>{category=='local'?this.props.appStore.arabic?'محلي':'local':category=='online'?this.props.appStore.arabic?'اونلاين':'Online':category=='Special'?this.props.appStore.arabic?'خاص':'Special':category}</Text>
              </Button>   )
        ...
        <Label style={{fontFamily: 'ralewaysemi',marginVertical: 10}}>
 {this.props.appStore.arabic?'الموقع':'Location'}
        </Label>
        <TouchableOpacity onPress={()=>this.setState({mapvis:true})} style={{flexDirection: 'row',alignItems: 'center'}}>

        <Icon active type="MaterialCommunityIcons" name='map-marker-outline' color="#eb144c" style={{color: '#eb144c'}}/>
        <Text style={{fontFamily: 'ralewaysemi'}}>{this.props.appStore.address}</Text>

        </TouchableOpacity>
        <Label style={{fontFamily: 'ralewaysemi',marginVertical: 10}}>
 {this.props.appStore.arabic?'المسافة':'Distance'} ({this.state.value}{this.props.appStore.arabic?'كم':'km'})
        </Label>
        <Slider
 ...
 {this.props.appStore.arabic?'القسم':'Categories'}
        ...
        {this.state.namedata=='Business'?  <RNPicker
        ...
        arabic={this.props.appStore.arabic}
                   ...
                   />:this.state.sec=='local'?  <RNPicker
                              ...
                              dropDownImageStyle={Styles.dropDownImageStyle}
                              selectedValue={(index, item) => this._selectedValue(index, item)}
                            />:this.state.sec=='Special'?      <RNPicker
                                           dataSource={this.state.specialarr}
                                           dummyDataSource={this.state.specialarr}
                                           arabic={this.props.appStore.arabic}
                                           arabichold={"اختر قسم او صنف"}
