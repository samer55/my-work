  ...
  render() {
    ...
    return (
      ...
      <Icon name='ios-close-circle' size={40} color="#000000"/>
                   ...
                   <Body>
                     <Title style={{color:'black'}}>{this.props.appStore.arabic?'تعديل معلومات الشخصية':'Edit Personal Information'}</Title>
                   </Body>
      ...
      <Spinner
           ...
           />
             ...
             <Label style={[gStyle.text[theme],{color:'#eb144c'}]}>{this.state.errMsg}</Label>
               ...
               <Item  fixedLabel>
                 <Label style={[gStyle.text[theme],{fontFamily: 'cairobold'}]}>{this.props.appStore.arabic?'الاسم':'Name'}</Label>
                 <Input  style={[gStyle.text[theme],{fontFamily: 'cairoreg'}]}  onChangeText={v=>this.setState({name:v})} value={this.state.name}/>
               </Item>
               <Item  fixedLabel>
                 <Label style={[gStyle.text[theme],{fontFamily: 'cairobold'}]}>{this.props.appStore.arabic?'البريد الالكتروني':'Email Address'}</Label>
                 <Input  style={[gStyle.text[theme],{fontFamily: 'cairoreg'}]}  onChangeText={v=>this.setState({email:v})} value={this.state.email}/>
               </Item>
         ...
         >
               ...
               >
                 <Text style={[gStyle.button,gStyle.text['dark']]}>{this.props.appStore.arabic?'تعديل':'Edit'}</Text>
               </Button>
               </View>
             </Content>
...
this.inc()
  ...
  _handleSave = () => {
      ...
      if (this.state.name.length < 5) {
        this.setState({ errMsg: this.props.appStore.arabic?'الاسم يجب ان يكون 5 احرف على الاقل..':"Your name should be at least 5 character" })
      }
      else {
        this.setState({ errMsg:this.props.appStore.arabic?'حفظ الاسم..': "Saving name.." })
        firebaseApp.firestore().collection('users').doc(firebaseApp.auth().currentUser.uid).update({name:this.state.name})
        firebaseApp.firestore().collection('profiles').doc(firebaseApp.auth().currentUser.uid).update({user:this.state.name})
        firebaseApp.auth().currentUser.updateProfile({ displayName: this.state.name })

        .then(() => {
          this.setState({ errMsg: this.props.appStore.arabic?'تم تغيير الاسم بنجاح':"Name changed successfully" })
        })
      ...
      if (this.state.email.length == 0) {
        this.setState({ errMsg: this.props.appStore.arabic?'الرجاء ادخال بريدك الالكتروني':"Enter your email address" })
      }
      else {
        this.setState({ errMsg:this.props.appStore.arabic?'جاري حفظ البريد..': "Saving email address.." })
        firebaseApp.firestore().collection('users').doc(firebaseApp.auth().currentUser.uid).update({email:this.state.email})

          firebaseApp.auth().currentUser.updateEmail(this.state.email)
          .then(() => {
            this.setState({ errMsg: this.props.appStore.arabic?'تم تغيير البريد بنجاح':"Email address changed successfully" })
          })
          .catch((error) => {
