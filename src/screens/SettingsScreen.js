...
setNames(theme=='dark'?false:true)
    ...
    useEffect(() => {
   /** handleWidgets */
   firebaseApp.firestore().collection('users').doc(firebaseApp.auth().currentUser.uid).onSnapshot(onCollectionUpdate)
...
<Icon name='ios-close-circle' size={40} color="#000000"/>
             ...
             <Body>
               <Title style={{color:'black'}}>{arabic?'الاعدادات':'Settings'}</Title>
             </Body>
...
<StatusBar hidden={true} />
      ...
      <ListItem Button  onPress={()=>navigation.navigate('EditPro',{name:names,email:mail})} icon>
        ...
        <Body>
          <Text style={gStyle.text[theme]}>{arabic?'تعديل المعلومات الشخصية':'Edit my Personal Information'}</Text>
        </Body>
        <Right>
          ...
          <Icon active name="arrow-forward"/>
              ...
              <ListItem icon button onPress={()=>navigation.navigate('Secure')}>
                ...
                <Body>
                  <Text style={gStyle.text[theme]}>{arabic?'تعديل كلمة المرور':'Edit password'}</Text>
                </Body>
                <Right>
                  <Text style={gStyle.text[theme]}></Text>
