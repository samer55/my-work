...
fetch('https://onesignal.com/api/v1/notifications',
 {
   ...
   body: JSON.stringify(
   {
     ...
     headings: {"en": `${firebaseApp.auth().currentUser.displayName} Order your Service`,"ar":`${firebaseApp.auth().currentUser.displayName} طلب خدمتك`},

     android_sound: "fishing",
     data: {"puid": newPostKey, "new_message":true},
     ios_sound: "fishing.caf",
     contents: {"en": `${firebaseApp.auth().currentUser.displayName} Order your Service:
     ${this.state.data.servicetitle}`,"ar":`${firebaseApp.auth().currentUser.displayName} طلب خدمتك:
...
header:null
  ...
  fav = () => {
   ...
   liked.push(firebaseApp.auth().currentUser.uid)
    ...
    firebaseApp.firestore().collection('Services').doc(this.state.data.postuid).update({nooflike:liked})
  ...
  return (
    <ThemeContext.Consumer>
      {theme => (
        ...
        <Text style={[styles.time,{color:'#fff'}]}>{this.props.appStore.arabic?'احجز الان':'Book Now'}</Text>
        </Button>
        </View>
...
<TouchableOpacity onPress={this.isInArray(liked, firebaseApp.auth().currentUser.uid)?this.unfav:this.fav}  style={{justifyContent: 'center',alignItems: 'center'}}>

<FontAwesome name={this.isInArray(liked, firebaseApp.auth().currentUser.uid)?'heart':'heart-o'} size={20} style={{marginRight: 10}} color="red"/>
</TouchableOpacity>
<Button onPress={this.openPanel} block style={{backgroundColor: '#eb144c',justifyContent: 'center',alignItems: 'center',padding: 20,alignSelf: 'flex-end',borderRadius: 10}}>
<Text style={[styles.time,{color:'#fff'}]}>{this.props.appStore.arabic?'احجز الان':'Book Now'}</Text>
</Button>
...
<MaterialIcons name='access-time' size={15} style={{marginRight: 5}} color="black"/>
            ...
            <Text style={[styles.type,{color:'black',textAlign: 'left'}]}  numberOfLines={1}>{this.props.appStore.arabic?this.state.data.time.replace('AM','صباحا').replace("PM", "مساءا"):this.state.data.time} - {this.props.appStore.arabic?this.state.data.end.replace('AM','صباحا').replace("PM", "مساءا"):this.state.data.end}</Text>
</View>
{true?  <View style={{flexDirection: 'row',alignItems: 'center',justifyContent: 'space-around',alignSelf: 'flex-start',marginVertical: 5,width: '100%'}}>
...
{this.state.data.Do}
                ...
                {this.state.data.backimg.length>0?<View style={[styles.cards,{width: Dimensions.get('window').width-10,height: 200}]}>
                <Text style={[gStyle.det,{
                  color:'#000000',marginVertical: 5
                }]}>{this.props.appStore.arabic?'الصور':'Images'}</Text>
                <Lightbox underlayColor="white">

                  <Image style={{width: Dimensions.get('window').width,height: '90%',resizeMode: 'contain'}} source={{uri:this.state.data.backimg}}/>
                  </Lightbox>
                  </View>:null}
        ...
        {this.state.reviewsarr.length>0&&this.state.reviewsarr.map((d)=>{
          return(<ListItem >
              ...
              <Body>
                ...
                <Text style={{fontFamily: 'raleway',marginVertical: 10,textAlign: 'center'}}>{d.comment}</Text>
              </Body>
              <Right>
              <Rating imageSize={15}   readonly
                startingValue={d.ratingnum} style={styles.rating} />
                  ...
                  <View style={{marginVertical: 3,flexDirection: 'column',paddingVertical: 10,flex:1,paddingHorizontal:10}}>
                        <Text style={[gStyle.text[theme], gStyle.Titleleft,{alignSelf: 'flex-start'}]}>{this.props.appStore.arabic?'خدمات بواسطة':'Services by'} {this.state.data.user}</Text>

                        <ShowScroller arabic={this.props.appStore.arabic} navigation={this.props.navigation} dataset={this.state.byme}  type="service"/>
                  </View>
            ...
            {!this.state.swipeablePanelActive&& (this.state.userdat.whats||this.state.userdat.phone)?<Fab
...
{this.props.appStore.arabic?'تفاصيل وملاحظات':'Notes & Details'}
            ...
            <Textarea rowSpan={5} style={gStyle.text[theme]} value={this.state.noteto} onChangeText={(noteto)=>this.setState({noteto})} bordered placeholder={this.props.appStore.arabic?'احتاج لتصليح جهازي المحمول..':"I need you to repair my laptop it's broken.."} />
          </Form>
