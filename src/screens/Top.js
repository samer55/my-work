...
this.displayCategories()
  ...
  displayCategories = () => {
    ...
    ref.once('value').then(snapshot => {
        ...
        this.setState({ dataArray: Object.values(sorted),searched:Object.values(items),spinner:false},function(){
    ...
    return (
      ...
      <Spinner
            ...
            textContent={'جاري التحميل...'}
             textStyle={{color:'#fff'}}
           />
<Text style={[gStyle.text[theme], , gStyle.Title,{alignSelf: 'flex-end',marginHorizontal: 20}]}>اعلى 5 لاعبين</Text>


        <List style={{width}}>
        <FlatList
   data={this.state.dataArray.slice(0, 5)}
   renderItem={({ item }) => (
     <ListItem thumbnail>
       <Left>
       ...
       <Right >
         <TouchableOpacity>
           <Text style={gStyle.text[theme]}>{item.balance} نقطة</Text>
         </TouchableOpacity>
