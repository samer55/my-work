...
user= firebaseApp.auth().currentUser.displayName;
  ...
  return (
    ...
    >
    {status.length>0?<View style={{height: 50,width:Dimensions.get('window').width,backgroundColor: status=='جاري الرد'?'green':status=='تم النشر'?'green':'red'}}>
...
thirdno={this.state.thirdno}
             lie={data.liegame}
             first={data.first}
             challenge={data.first=="red"?'قبل التحدي':data.first=='steelblue'?'اختار الصراحة':data.first=='yellow'?'اختار باب الحب':'اختار الجرأة'}
...
<Text style={[gStyle.text[theme],  gStyle.p,{alignSelf: 'flex-end',marginHorizontal: 20,fontFamily: 'cairoreg',color: '#eb144c'}]}>{this.state.status}</Text>

{
  data.liegame&&liesuser&&liesuser.length>0?
    <Content>
    <Text style={[gStyle.text[theme],  gStyle.p,{alignSelf: 'flex-end',marginHorizontal: 20,fontFamily: 'cairoreg',color: '#FFF'}]}>اللاعبين</Text>
  <List style={{width}}>
  <FlatList
  data={liesuser}
