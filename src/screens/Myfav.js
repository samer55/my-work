...
import {   List, ListItem, Left, Body, Right, Thumbnail,Text,Tab, Tabs, ScrollableTab} from 'native-base';
...
console.log(this.state.dataArray);
  ...
  componentDidMount(){
    ...
    firebaseApp.firestore().collection("profiles")
.where("nooflike", "array-contains", firebaseApp.auth().currentUser.uid).onSnapshot(this.Jobsupdate)
  //  firebaseApp.firestore().collection('Orders').where('customeruid','==',firebaseApp.auth().currentUser.uid).onSnapshot(this.request)
  firebaseApp.firestore().collection("Services")
.where("nooflike", "array-contains", firebaseApp.auth().currentUser.uid).onSnapshot(this.servicess)
  ...
  this.setState({dataSources:boards});
  }
  servicess = (querySnapshot) => {
    const boards = [];
  ...
  this.setState({servicesarr:boards});
  }
  getfavuser = (querySnapshot) => {
    firebaseApp.firestore().collection('profiles').where('writerId','in',querySnapshot).onSnapshot(this.Jobsupdate)
  ...
  this.setState({searched:Object.values(boards)});
  }
    create = (d) => {
      this.setState({
        ploading:true,
  ...
  render() {
    ...
    return (
      ...
      <Icon name='arrow-back' size={40} color="#000000"/>
                   ...
                   <Body>
                     <Title style={{color:'black'}}>{this.props.appStore.arabic?'المفضلة':'Favourite'}</Title>
                   </Body>
          ...
          >
            ...
            {this.state.dataSources.map((item, index) => {
              ...
              return(
                <ListItem onPress={()=>navigation.navigate('Userprofile',{id:item.writerId})} avatar>
                              <Left>
                              ...
                              <Right>
                                <Text note>{this.props.appStore.arabic?item.typeofservice=='local'?'محلي':'اونلاين':item.typeofservice}</Text>
                              </Right>
                            </ListItem>
          ...
          <Spinner
               ...
               <ShowScroller  arabic={this.props.appStore.arabic} navigation={this.props.navigation} dataset={this.state.servicesarr} type="latest" />
